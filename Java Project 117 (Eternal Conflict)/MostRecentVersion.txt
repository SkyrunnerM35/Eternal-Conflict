...
public class EternalConflict {
	...
	public EternalConflict() {
		...
		environments = new String[] {"NONE", "STPX", "ASTR", "PDFA", "PDFE", "FLBT", "PTRG"};
	}
	...
	public void applyEnvironmentalEffects(Ship ship) {
		...
		case "STPX":
			...
			System.out.println("  RADIANT HEAT");
			heatTarget(ship, 8);
			if(Math.random() < .1) {
		...
		case "FLBT":
			...
			double dodgeRate;
			for(int i = 0; i < 5; i++) {
				dodgeRate = Math.max(5 * ship.getEvasion(), .4);
				if(ship.hasStatusEffect("EMPSE")) {
					dodgeRate = .4;
				}
				if(Math.random() > dodgeRate) {
					if(ship.getShield() >= 6) {
						damageShip(ship, 2, 12, standardCrit, 5 * ship.getEvasion(), true, false, false);
					} else {
						damageShip(ship, 0, 12, standardCrit, 5 * ship.getEvasion(), true, false, false);
					}
				} else {
			...
			break;
		case "PTRG":
			System.out.println("PLANETARY RINGS");
			System.out.println("  SENSORY INTERFERENCE");
			System.out.println("    Evasion Rates are Effectively Doubled");
			System.out.println("    Missile Weapons are Disabled");
			environmentalEvasion = 2 * standardEvasion;
			break;
		}
		environmentTurns--;
	}
	...
	public void changeEnvironment() {
		...
		case "PTRG":
			System.out.println("PLANETARY RINGS");
			System.out.println("  The engagement has shifted to within one of the rings of a nearby gas");
			System.out.println("  giant. The extra debris is interfering with your sensors, making it that");
			System.out.println("  much harder to keep a bead on your enemy.");
		}
	...
	public void underAttack(Ship attacker) {
		...
		} else if(attacker.hasWeapon("ML") && !currentEnvironment.equals("PTRG") && attacker.getMunitions(0) > 0 && (!player.getPDState() || player.hasStatusEffect("EMPSE"))) {
			System.out.println("\nMISSILE LAUNCHER");
